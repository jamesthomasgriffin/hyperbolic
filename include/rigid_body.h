...
struct rigid_body_t {
  ...
  static rigid_body_t
  from_distribution_matrix(mat4 const &M); // Returns a rigid body at rest
  static scalar_t
...
inline rigid_body_t::velocity_t rigid_body_t::momentum() const {
...
inline mat4 mass_distribution_matrix_sphere_shell(float mass, float radius) {
  mat4 M = mat4{};
  mat4::value_type a = mass * powf(std::sinh(radius), 2) / 3;
  M[0][0] = M[1][1] = M[2][2] = a;
  M[3][3] = -mass - 3 * a;
  return M;
}
...
inline mat4 mass_distribution_matrix_sphere_shell(vec4 const &position,
  ...
  mat4 P = hyperbolic::transport_frame_to(mat4{1}, {position});
...
inline mat4 mass_distribution_matrix(rigid_body_t const &b) {
  ...
  vec4 const diagonal =
      glm::inverse(M) * vec4{b.distributional_inertia, b.total_mass};
...
inline rigid_body_t
rigid_body_t::from_distribution_matrix(mat4 const &dist_matrix) {
